...
public class SelectWepon : MonoBehaviour {
     ...
     *      3 : ナイフ・ハンドガン・フライパン
     */
    public int WeponLevel = 1;
    private int WeponLevelBuf = 1;
    ...
    public bool FlyPanEquipFlag { set; get; }
            ...
            for (int i = 0; i < Wepon.Length; i++) {
                ...
                if (WeponRect[i].localPosition.x == Main.x) {
                    ...
                    if (WeponImage[i].sprite == KnifeSprite) {
                        ...
                        WeponType = 1;
                    } else if(WeponImage[i].sprite == FlyPanSprite) {
                        WeponType = 2;
                        FlyPanEquipFlag = true;
                    }
                    /*
	...
	void Update () {
       ...
       */
       if(WeponLevel != WeponLevelBuf) {
        ...
        if (Input.GetAxis("Cross_Horizontal") <= -1 && !SlideFlag) {
            SlideFlag = true;
            PosFlag = true;
            Dir = 0;
        }
